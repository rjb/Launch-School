class BeerSong
  def verses(first, last = nil)
    last ||= first
    first.downto(last).map { |n| display_verse(n) }.join("\n")
  end
  alias :verse :verses
  ...
  def display_verse(n)
    "#{pluralize(n).capitalize} of beer on the wall, " +
    "#{pluralize(n)} of beer.\n" +
    "#{buy_phrase(n)}" +
    "#{pluralize(n - 1)} of beer on the wall.\n"
  end
